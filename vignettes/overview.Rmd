...
## (4) Update the Random Forests’ model by adding the new Class 3 labels back to existing training data, and then retrain and evaluate model performance.
...
### Identify clusters of cells greater than size (say 20 cells). Convert thresholded maps into individual objects, filtering out smaller objects. (Zeyu)
### Convert that to vectors, using the package function polygonize_labels.R. (Zeyu)
### Examine each retained cluster against PlanetScope imagery from the  same date. (Zeyu)

## 3. d data and create time-series images     (Vishal)

### April 25 - May 1: Train Random Forest model and Run predictions      (Yao-Ting)
### May 2 - May 8: Identify the highest likelihood predictions for each crop type (Zeyu)
### May 9 - May 15: Make presentations, and submit our project

## 4. Anticipated outcomes

### Random Forests’ model and predictions:We will train the Random   Forests’ Model using Class 1 labels, and get predictions of the crop field.
### Class 3 labels:Based on the predictions, we will delete the  invalid predictions, and keep the valid predictions as Class 3 labels.
